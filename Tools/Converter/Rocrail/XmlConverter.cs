...
namespace Converter.Rocrail
{
    public partial class XmlConverter
    {
        ...
        public List<StagingElement> StagingElements { get; } = new();
        public List<TextElement> TextElements { get; } = new();
        public List<CoElement> CoElements { get; } = new();

        public bool RunData(string data, string outputDirectory)
        {
        ...
        public bool Run(string planPath, string outputDirectory)
        {
            _outputDirectory = outputDirectory;
        ...
        private int GetThemeIdByType(TrackElement el)
        {
            if (el.ElementType == PlanItemT.Fb)
            {
                var elFb = el as FeedbackElement;
                if (elFb != null && elFb.IsCurve)
                    return 201;
            ...
            else if (el.ElementType == PlanItemT.StagingBlock)
            {
                return 152;
            }
            else if (el.ElementType == PlanItemT.Tx)
        ...
        private void ParseSbList()
        {
            StagingElements.Clear();

            var nodeSbList = _doc.SelectNodes("/plan/sblist");
            if (nodeSbList == null)
            {
                Trace.WriteLine("<sblist> is missing");
                return;
            }
            foreach (XmlNode node in nodeSbList)
            {
                var childs = node?.ChildNodes;
                if (childs == null || childs.Count == 0) continue;
                ...
                foreach (var itChild in childs)
                {
                    var el = new StagingElement();
                    var res = el.ParseXmlNode(itChild as XmlNode);
                    if (!res) continue;

                    StagingElements.Add(el);
                }
            }
        }
