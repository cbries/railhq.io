...
namespace libEsuEcos
{
    ...
    {
        private static bool IsXyz(string name, ICommand cmd)
        {
            if (string.IsNullOrEmpty(cmd?.Name)) return false;
            return cmd.Name.Equals(name, StringComparison.Ordinal);
        }

        public static bool IsGet(ICommand cmd)
        {
            return IsXyz("get", cmd);
        }

        public static bool IsSet(ICommand cmd)
        {
            return IsXyz("set", cmd);
        }
        
        public static bool IsRelease(ICommand cmd)
        {
            return IsXyz("release", cmd);
        }

        public static bool IsQueryObjects(ICommand cmd)
        {
            return IsXyz("queryObjects", cmd);
        }

        public static bool IsRequest(ICommand cmd)
        {
            return IsXyz("request", cmd);
        }
