    ...
    $.fn.w2menu = function (menu, options) {
        ...
        */
        ...
        if (menu === 'refresh') {
            ...
            $.fn.w2menuOver = function (event, index) {
                var $el = $(event.target).parents('tr');
                if (window.__stagingFbOverCallback)
                    window.__stagingFbOverCallback($el);
            };
            var html = '';
            if (options.search) {
                html +=
            ...
            for (var f = 0; f < options.items.length; f++) {
                ...
                if (mitem.hidden !== true) {
                    ...
                    if (txt != null && txt !== '' && !(/^-+$/.test(txt))) {
                        ...
                        '        onmouseover="if (' + (mitem.disabled === true ? 'true' : 'false') + ') return;' +
                        '               jQuery.fn.w2menuOver(event, \'' + f + '\');"' +
    ...
    w2grid.prototype = {
                ...
                for (var index = this.last.range_start - 1; index <= this.last.range_end - 1; index++) {
                    ...
                    for (var column = 0; column < this.columns.length; column++) {
                        ...
                        if (rec.w2ui.style != null && !$.isEmptyObject(rec.w2ui.style)) {
                            ...
                            // Ries: removed, when added it destroys the text-align attribute
                            //cell.attr('style', '');
                        }
                        // assign class
        ...
        refreshCell: function (recid, field) {
            ...
            if (rec.w2ui && rec.w2ui.style != null && !$.isEmptyObject(rec.w2ui.style)) {
                ...
                // Ries: removed, when added it destroys the text-align attributev
                //cell.attr('style', '');
            }
            // assign class
