...
class TextfieldElement {
    ...
    install(options = {}) {
        ...
        const tplEl = $('.tplElEditorRoot');
        const el = tplEl.clone();
        el.removeClass("tplElEditorRoot");
        el.addClass("elEditorRoot");
        this.__element = el;
        if(this.__uniqueId == null)
            this.__uniqueId = generateUniqueItemIdentifier();
                ...
                if (event.color != null) {
                    ...
                    switch (event.object.id) {
                        case "cmdRemove": { 
                            ...
                            window.planfield.removeControl(uniqueId);
                    ...
                    window.planfield.sendControl(ctxRootEditorElement);
                }
            }
        });
    }
    ...
    __initEditorForTextElements() {
        ...
        const el = this.__element;
        let recentTextEditor = null;
        ...
        el.keydown(function (e) {
            if (!e.shiftKey) {
                if (e.keyCode === constKeyEnter || e.keyCode === constKeyEscape) {
                    ...
                    if (recentTextEditor) {
                        recentTextEditor.blur();
                        recentTextEditor.attr("contenteditable", false);
                        window.planfield.sendControl(recentTextEditor);
                    }
                }
            }
        });
        ...
        const elEditor = el.find('.elEditor');
